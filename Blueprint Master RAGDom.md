...
* **Contrainte Render 512 Mo (hébergements FREE) → `RAGDOM_LOW_MEMORY=true` :** l'encodeur d'embeddings ONNX (~300 Mo de pic) n'est **jamais** chargé et la recherche passe en **BM25 seul**. Le scan de page passe de **300 à 150 DPI uniquement dans ce mode**; deskew, Sauvola, rapid-layout, rapid-latex-ocr et rapid-table sont sautés. RapidOCR est également **désactivé par défaut** pour éviter un troisième restart après deux échecs live; seul l'opt-in explicite `RAGDOM_LOW_MEMORY_OCR=true` autorise son chargement. Sans opt-in, une page scannée reste image-only à 150 DPI dans la working DB et reçoit le badge/benchmark `ImageOnly-LowMemory`; elle peut être retraitée ultérieurement sur un environnement plus large ou avec l'opt-in. Une page native conserve son extraction texte PyMuPDF sans ONNX. L'OCR VLM de page entière en mode `auto` est désactivé afin d'éviter l'image/base64 pleine page; seul `RAGDOM_VLM_PAGE_OCR=true` constitue un opt-in explicite. `page_scans` persiste le DPI réel (150 ou 300) afin d'aligner dimensions et BBox. Les crops/images restent conservés (`raw_binary`/`needs_vlm`) pour requalification ultérieure. Les bases pré-construites conservent leurs vecteurs pour un futur hébergement plus large. `llama-cpp-python` (LLM GGUF local) est exclu de l'image web — fallback LLM web = Ollama/API.
...
Le routeur FastAPI est monté sous `/api/validation`. Il fournit prévisualisation du scope, création/liste/détail des runs, lecture et mise à jour des copies, snapshots logiques/restauration, diffs page et run, rapport auditable, rattachement des benchmarks, accept/reject/cancel au niveau run et gestion/diagnostic des profils embeddings. L'inspecteur sert scans et binaires d'artefacts depuis la version baseline/working demandée et lit TOC, curriculum et benchmarks dans la working DB; un schéma TOC legacy sans `parent_id` reste lisible. Pour un run multi-document, `document_id` désambiguïse les numéros de page identiques.

### **9.2 Isolation, snapshots, diff et publication**

`POST /runs/{id}/execute` résout chaque source PDF officielle selon un contrat portable et local-first : un chemin direct existant vers un PDF reste accepté; sinon, un ancien chemin absolu Windows/Linux est relocalisé en extrayant son suffixe sous `sources/` et en le rattachant au `SOURCES_DIR` courant. Le fallback par nom de fichier n'est accepté que s'il existe une correspondance unique sous cette racine; un nom ambigu ou une source absente bloque explicitement le run. Le chemin résolu est persisté uniquement dans la working DB, sans réécrire l'officielle. L'exécution purge/ré-exécute ensuite toutes les couches et persiste batches/jobs/scans/chunks/artefacts/benchmarks **dans la seule copie physique**. Après terminaison, `working_json` est rafraîchi depuis cette copie et les diffs comparent baseline/working avec empreintes SHA-256. Les états `CREATED/QUEUED/RUNNING/COMPLETED/BLOCKED/FAILED/CANCELLED` et opération, batch(s), progression et erreur sont exposés par liste, détail et rapport. Cancel supprime tous les jobs actifs/reprenables de la copie et stoppe ses batchs : recovery ne peut pas les recréer et un run `CANCELLED` ne peut pas être ré-exécuté.

L'acceptation est atomique et porte sur le **run entier** : verrou d'écriture, contrôle anti-concurrence par `baseline_hash` sur toutes les lignes effectivement promues — chunks, artefacts, scans et benchmarks compris —, protection des éditions humaines, contrôle de tous les propriétaires et liens cross-document, puis remplacement coordonné des pages. Une modification officielle concurrente d'un scan ou benchmark produit donc un 409 sans écrasement. Le rejet est lui aussi au niveau run et ne publie rien. Il n'existe pas d'accept/reject page par page ; seule la restauration de copie peut cibler une page.

### **9.3 Schéma, curriculum et espace vectoriel**

Les migrations additives **005/006/007** apportent runs/pages/events/snapshots, provenance benchmark/artefact, profils embeddings, ownership curriculum par document, hash de baseline, unicité d'un job actif par page et cycle d'exécution physique. La **007**, additive et idempotente, ajoute `working_db_filename`, `operation`, `batch_id`, `batch_ids_json`, `execution_status`, `progress_current/total`, `error_log`, `started_at/completed_at`, ainsi que les index de statut et d'unicité de working DB. Le curriculum devient sûr en multi-livres : terms, programs, assessments et links appartiennent explicitement à un document ; les données legacy ambiguës ne sont jamais backfillées au hasard.
...
Le Studio est intégré à Automation : builder avec preview obligatoire, sélecteur de scope multi-base/multi-document, liste paginée, deep-link `db/run/doc/page`, progression, inspecteur (chunks, artefacts et scans baseline/working, TOC, curriculum, benchmarks), diff, restauration et modale de confirmation avant acceptation. Le suivi interroge uniquement le run ouvert toutes les 5 secondes : **polling ciblé, pas de SSE Validation dédié**. Le flux SSE Pipeline reste indépendant. Si l'authentification interrompt l'accès, le chemin interne complet (query et hash inclus) est conservé via `next`; seules `/`, `/library` et `/automation` sont acceptées comme destinations, toute redirection externe retombant sur `/automation`.

En readonly, le frontend désactive les actions et le middleware masque toutes les routes `/api/validation` en 404. En mode administré, session ou Bearer est exigé. La sécurité complète inclut DTO stricts, nom de base sanitisé, validation de scope, ownership document/page, interdiction des références cross-document, protection `is_human_edited`, conflit optimiste et transaction unique de publication. Le namespace `validation_test_` est réservé aux routes Validation : il est caché de la découverte/health, non exportable/duplicable et refusé comme cible des mutations génériques library/curriculum/pipeline.
...
Preuves finales rejouées après exécution end-to-end et durcissement Render 512 Mo : pytest **165/165** normal et **165/165** faible mémoire, Vitest **17/17**, build TypeScript strict + Vite **8.2.2** vert (**3 711 modules**), React Router DOM **7.18.2**, `npm audit` **0 vulnérabilité**. Après deux restarts live, RapidOCR est désormais opt-in en faible mémoire afin d'éviter un troisième restart; la recette doit confirmer qu'une page scannée sans opt-in reste disponible avec le statut `ImageOnly-LowMemory`.
