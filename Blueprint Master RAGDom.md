...
* **Contrainte Render 512 Mo (hébergements FREE) → `RAGDOM_LOW_MEMORY=true` :** l'encodeur d'embeddings ONNX (~300 Mo de pic) n'est **jamais** chargé et la recherche passe en **BM25 seul**. Le scan de page passe de **300 à 150 DPI uniquement dans ce mode**; deskew, Sauvola, rapid-layout, rapid-latex-ocr et rapid-table sont sautés. RapidOCR reste activé par défaut : pour une page scannée, il est le **seul moteur ONNX chargé**; `RAGDOM_LOW_MEMORY_OCR=false` le désactive. Une page native extrait son texte sans charger aucun OCR. L'OCR VLM de page entière en mode `auto` est désactivé afin d'éviter l'image/base64 pleine page; seul `RAGDOM_VLM_PAGE_OCR=true` constitue un opt-in explicite. `page_scans` persiste le DPI réel (150 ou 300) afin d'aligner dimensions et BBox. Les crops/images restent conservés (`raw_binary`/`needs_vlm`) pour requalification ultérieure. Ce durcissement fait suite à **deux restarts live Render** après le lancement de la page 1; la prochaine recette doit confirmer un run de la page 1 sans restart. Les bases pré-construites conservent leurs vecteurs pour un futur hébergement plus large. `llama-cpp-python` (LLM GGUF local) est exclu de l'image web — fallback LLM web = Ollama/API.
...
Preuves finales rejouées après exécution end-to-end et durcissement Render 512 Mo : pytest **165/165** normal et **165/165** faible mémoire, Vitest **17/17**, build TypeScript strict + Vite **8.2.2** vert (**3 711 modules**), React Router DOM **7.18.2**, `npm audit` **0 vulnérabilité**. La prochaine recette live doit confirmer l'exécution de la page 1 sans restart.
