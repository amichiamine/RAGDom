...
- Render FREE : 512 Mo → `RAGDOM_LOW_MEMORY=true` obligatoire. Le scan passe alors de 300 à **150 DPI**; deskew, Sauvola, rapid-layout, rapid-latex-ocr et rapid-table sont sautés. RapidOCR est **désactivé par défaut** afin d'éviter un troisième restart après deux échecs live; `RAGDOM_LOW_MEMORY_OCR=true` est l'unique opt-in pour le charger. Sans opt-in, une page scannée reste image-only à 150 DPI dans la working DB et porte le badge/benchmark `ImageOnly-LowMemory`; la retraiter plus tard sur un environnement plus large ou avec l'opt-in. Une page native conserve PyMuPDF sans ONNX. `RAGDOM_VLM_PAGE_OCR=auto` est désactivé dans ce mode; utiliser `true` seulement comme opt-in explicite. `page_scans` persiste le DPI réel.
