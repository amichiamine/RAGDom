...
CREATE TABLE IF NOT EXISTS processing_benchmarks (
    ...
    document_id         TEXT NOT NULL,
    page_number         INTEGER NOT NULL,
    engine_used         TEXT NOT NULL,          -- ex: "RapidOCR", "rapid-latex-ocr", "PyMuPDF4LLM", "ImageOnly-LowMemory"
...
* **Seuil de pertinence (Anti-Hallucination — V3.1, scores bruts non-ordinaux) :** le RRF ordonne mais ne filtre pas (un rang 1 passe toujours, même sémantiquement nul). Un chunk n'est éligible que si (distance cosinus sqlite-vec ≤ 0.45) OU (score `bm25(search_index)` ≤ -0.3 — score négatif SQLite = pertinent ; calibré en conditions réelles Phase 2 : un match significatif vaut ≤ -0.3, -1.5 rejetait les corpus courts, tri `ORDER BY bm25(...) ASC`, plus petit = meilleur). **Chaque canal est filtré par son propre seuil AVANT le calcul des rangs et de la fusion : un voisin vectoriel hors seuil ne contribue pas au RRF d'un chunk sauvé par BM25, et réciproquement. Les rangs BM25 sont uniques par `chunk_id` et conservent la meilleure ligne FTS (un artefact lié peut créer une deuxième ligne d'index pour le même chunk).** Si aucun chunk n'est éligible, l'agent IA répond obligatoirement : *"Je ne trouve pas d'informations pertinentes dans la bibliothèque actuelle."* En mode `fts5-fallback`, seul le critère BM25 s'applique. Les deux seuils sont stockés dans `app_settings` (`ragdom_config.sqlite`) et ajustables sans redéploiement.
...
* **Déclenchement :**
  ...
  * **Mode Render 512 Mo (`RAGDOM_LOW_MEMORY=true`) :** le scan passe de **300 à 150 DPI uniquement dans ce mode**. Deskew et Sauvola sont sautés (binarisation Otsu légère), comme rapid-layout, rapid-latex-ocr et rapid-table. RapidOCR est désactivé par défaut afin d'éviter un troisième restart après deux échecs live; seul `RAGDOM_LOW_MEMORY_OCR=true` autorise son chargement. Le mode VLM `auto` est désactivé pour éviter l'encodage/base64 d'une image pleine page; l'OCR VLM n'est autorisé que par l'opt-in explicite `RAGDOM_VLM_PAGE_OCR=true`.
...
* **Repli hors-ligne et conservation :** en mode normal, RapidOCR (modèles PP-OCRv4 embarqués dans le paquet) reste le premier extracteur des scans et le VLM ne fait que le **surclasser** quand il répond. En faible mémoire sans opt-in OCR, une page scannée reste image-only à 150 DPI dans la working DB; ses images/crops sont conservés pour une requalification ultérieure sur un environnement plus large ou avec l'opt-in.
* **Cycle de vie des moteurs en 512 Mo :** aucun moteur ONNX n'est chargé par défaut. `RAGDOM_LOW_MEMORY_OCR=true` active explicitement RapidOCR uniquement lorsqu'une page est scannée. Sans opt-in, le badge UI et `processing_benchmarks.engine_used` valent `ImageOnly-LowMemory`. Une page native utilise son texte PyMuPDF sans ONNX. `page_scans` persiste le DPI réellement utilisé (150 ou 300), et les BBox sont calculées dans ce même référentiel.
* **Flags :** `RAGDOM_VLM_PAGE_OCR` — `auto` (défaut normal, neutralisé si `RAGDOM_LOW_MEMORY=true`) | `false` (Tier 1 seul partout) | `true` (opt-in explicite, y compris sur 512 Mo). `RAGDOM_LOW_MEMORY_OCR` — désactivé par défaut; seule la valeur `true` active RapidOCR dans le profil faible mémoire. Le moteur ou statut retenu est reporté dans `engine_used` (`VLM-OCR` en cas de succès, `ImageOnly-LowMemory` sans OCR sur une page scannée en faible mémoire).
...
RAGDOM_VLM_PAGE_OCR=auto   # OCR VLM pleine page (§3.6.1) : auto | false | true. En low-memory, auto est désactivé; true = opt-in explicite.
...
RAGDOM_LOW_MEMORY=false   # true (≤512 Mo) : scan 150 DPI; BM25 seul; moteurs ONNX désactivés par défaut; VLM pleine page auto désactivé.
RAGDOM_LOW_MEMORY_OCR=false  # Profil 512 Mo : RapidOCR désactivé par défaut; true = opt-in explicite sur page scannée. Sans opt-in : ImageOnly-LowMemory; page native : PyMuPDF sans ONNX.
...
Un run capture pour chaque `(document_id,page_number)` un `baseline_json`, son `baseline_hash` et un `working_json` séparé. `POST /runs` crée par `Connection.backup` une base `validation_test_<run>.sqlite`; `/execute` exécute réellement reprocess/pipeline, jobs, scans, artefacts et benchmarks dans cette seule copie. La résolution de `source_path` est portable et local-first : (1) un chemin direct existant et suffixé `.pdf` est accepté; (2) sinon, les séparateurs Windows/Linux sont normalisés et le dernier suffixe après `/sources/` est rattaché au `SOURCES_DIR` courant, avec contrôle de confinement et d'existence; (3) en dernier recours, le nom de fichier doit avoir une correspondance exacte unique sous `SOURCES_DIR`. Zéro ou plusieurs correspondances produisent `BLOCKED`. Les chemins résolus sont écrits seulement dans `validation_test_<run>.sqlite`; la DB officielle n'est pas normalisée ni mutée. Création, édition de copie, snapshot, restauration, diff, rejet et annulation renvoient ou garantissent `official_mutated:false`. Les snapshots publiquement créables sont **logiques uniquement** : la base sait stocker la valeur historique `physical`, mais l'API la refuse car la restauration ne sait restaurer sans risque que `working_json`.
...
État final vérifié sur `feat/validation-integration` après le durcissement Render 512 Mo : pytest **165/165** en mode normal et **165/165** avec `RAGDOM_LOW_MEMORY=true`; Vitest **17/17**; build TypeScript strict + Vite **8.2.2** vert (**3 711 modules**); `react-router-dom` **7.18.2**; `npm audit` **0 vulnérabilité**. Après deux recettes live ayant provoqué un restart au lancement de la page 1, le profil faible mémoire désactive désormais RapidOCR par défaut afin d'éviter un troisième restart; une page scannée sans opt-in est conservée et tracée `ImageOnly-LowMemory`.
