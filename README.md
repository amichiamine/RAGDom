...
**Mode Render 512 Mo final :** avec `RAGDOM_LOW_MEMORY=true`, le scan passe seul de **300 à 150 DPI**. Deskew, Sauvola, rapid-layout, rapid-latex-ocr et rapid-table sont sautés. RapidOCR reste activé par défaut et devient le **seul moteur ONNX chargé**, uniquement pour une page scannée; `RAGDOM_LOW_MEMORY_OCR=false` permet de le désactiver. Une page native extrait son texte sans charger aucun OCR. L'OCR VLM de page entière en mode `auto` est désactivé et seul l'opt-in explicite `RAGDOM_VLM_PAGE_OCR=true` le réactive. `page_scans` persiste le DPI réellement utilisé (150 ou 300), avec les dimensions et BBox dans le même référentiel. Ce durcissement fait suite à **deux restarts live Render** après le lancement de la page 1; la prochaine recette live doit confirmer un run de la page 1 sans restart.
