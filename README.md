...
Les états d'exécution sont `CREATED → QUEUED/RUNNING → COMPLETED`, ou `BLOCKED` si un PDF source officiel manque ou reste ambigu, `FAILED` en cas d'échec et `CANCELLED` après annulation ciblée. Lors de `POST /runs/{id}/execute`, un chemin direct existant vers un PDF est accepté en priorité pour préserver le mode local-first. Sinon, les anciens chemins absolus Windows/Linux sont relocalisés par leur suffixe sous `sources/` vers le `SOURCES_DIR` courant; le fallback par nom de fichier n'est accepté que pour une correspondance unique. Le chemin résolu est mis à jour uniquement dans la working DB, jamais dans l'officielle. Le détail et le rapport exposent copie, opération, batch(s), progression et erreur ; le frontend poll uniquement le run ouvert. Cancel supprime de la copie tous les jobs actifs/reprenables et stoppe les batchs : recovery ne peut pas ressusciter le run et `/execute` refuse ensuite sa reprise. Le rejet supprime la copie, son WAL et son SHM.

L'inspecteur charge depuis la bonne base les scans et binaires d'artefacts baseline/working, et expose depuis la working DB le TOC, le curriculum et les benchmarks. Il reste compatible avec un TOC legacy sans colonne `parent_id`. L'acceptation contrôle sous transaction le hash optimiste de toutes les lignes promues — `page_scans` et `processing_benchmarks` inclus — ainsi que les éditions humaines, puis promeut les seules pages du scope avant de supprimer la copie.

La migration additive/idempotente **007** complète 005/006 avec `working_db_filename`, `operation`, `batch_id`, `batch_ids_json`, `execution_status`, progression, erreur et dates d'exécution, plus ses index de statut et d'unicité de working DB. Le namespace `validation_test_` est masqué de la découverte/health, non exportable/duplicable et refusé par les mutations génériques library/curriculum/pipeline ; seules les routes Validation pilotent ces copies. Une requalification **mutante** avec `run_id` est autorisée uniquement sur la working DB physique d'un run `COMPLETED`, bornée aux pages du run, puis resynchronisée dans `working_json` ; elle ne cible jamais la base officielle.
...
**Mode Render 512 Mo final :** avec `RAGDOM_LOW_MEMORY=true`, le scan passe seul de **300 à 150 DPI**. Deskew, Sauvola, rapid-layout, rapid-latex-ocr et rapid-table sont sautés. RapidOCR est désormais **désactivé par défaut** pour éviter un troisième restart Render après deux échecs live; l'activer exige l'opt-in explicite `RAGDOM_LOW_MEMORY_OCR=true`. Sans opt-in, les pages scannées restent image-only à 150 DPI dans la working DB, avec badge et benchmark `ImageOnly-LowMemory`; elles peuvent être retraitées ultérieurement sur un environnement plus large ou avec l'opt-in. Les pages natives conservent l'extraction texte PyMuPDF sans ONNX. L'OCR VLM de page entière en mode `auto` est désactivé et seul l'opt-in explicite `RAGDOM_VLM_PAGE_OCR=true` le réactive. `page_scans` persiste le DPI réellement utilisé (150 ou 300), avec les dimensions et BBox dans le même référentiel.

**Preuves finales release :** pytest **165/165** en mode normal et **165/165** avec `RAGDOM_LOW_MEMORY=true`; Vitest **17/17**; build TypeScript strict + Vite **8.2.2** vert (**3 711 modules**); `npm audit` **0 vulnérabilité**.
