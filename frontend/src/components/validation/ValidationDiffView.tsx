...
interface Props { diff: ValidationDiff | null | undefined; db: string }

export default function ValidationDiffView({ diff, db }: Props) {
  const { t } = useLanguage()
      ...
      {diff.artifacts.length > 0 && (
        <div className="validation-diff-section">
          ...
          <div className="validation-artifact-grid">{diff.artifacts.map(item => {
            ...
            return artifact ? <article key={`${item.artifact_id}-${item.change}`} className={`validation-artifact-diff is-${item.change}`}><span className="badge badge-subtle">{t(`validation.diff.${item.change}`)}</span><ArtifactRenderer artifact={artifact} fallbackImageUrl={artifact.has_binary ? api.library.getArtifactBinaryUrl(db, artifact.id) : undefined} /></article> : null
          })}</div>
        </div>
      )}
